...
WHERE funcionario.estado_id = funcionario.id;
...
AND (estado.nome = 'RIO GRANDE DO SUL' OR estado.nome = 'PARANÁ' OR estado.nome = 'SANTA CATARINA')
...
R = Utilizaremos o JOIN quanto tivermos que relacionar registro de mais de 01 tabela. Quaisquer tabelas que possuem uma PK relacionando a uma FK.
2.	No caso do JOIN, o que é condição de junção. Qual cuidado devemos ter. 
R = Trata-se de junção de produto cartesiano. E, devemos ter cuidado com as colunas que serão referenciadas.
...
R = É a multiplicação de pares ordenados envolvendo conjuntos distintos. Cada item de um conjunto é relacionado para cada item de outro conjunto. Sua relação com o JOIN é referente a forma como o JOIN funciona no SQL
